...
pub struct ECDSAMultiCircuit<F> {
    ...
    pub pk: Vec<Option<Secp256k1Affine>>,
...
#[cfg(test)]
#[test]
fn bench_secp256k1_ecdsa_multi() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in bench_params_reader.lines() {
        ...
        for _ in 0..2 {
            ...
            r_vec.push(Some(r));
            s_vec.push(Some(s));
            msg_hash_vec.push(Some(msg_hash));
            pubkey_vec.push(Some(pubkey));
            G_vec.push(G);
        }
